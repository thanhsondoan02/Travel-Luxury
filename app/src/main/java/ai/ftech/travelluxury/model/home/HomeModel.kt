package ai.ftech.travelluxury.model.home

class HomeModel {

    companion object {
        val HOME_MODEL = HomeModel()
    }

    var cityHotelList: List<City>? = null

    fun mockData() {
        val city0 = City(
            0,
            "Ha Long",
            "https://firebasestorage.googleapis.com/v0/b/travel-luxury-4f5df.appspot.com/o/ha_long.png?alt=media&token=820c9948-929c-41a3-b376-7e1df5049ed2"
        )
        val city1 = City(
            1,
            "Vung Tau",
            "https://firebasestorage.googleapis.com/v0/b/travel-luxury-4f5df.appspot.com/o/vung_tau.png?alt=media&token=e20da326-27e3-4461-8748-4baff841f585"
        )
        val city2 = City(
            2,
            "Da Nang",
            "https://firebasestorage.googleapis.com/v0/b/travel-luxury-4f5df.appspot.com/o/da_nang.png?alt=media&token=db3cac9d-e959-4970-b406-b3d4c84477cc"
        )
        val city3 = City(
            3,
            "Ha Long",
            "https://firebasestorage.googleapis.com/v0/b/travel-luxury-4f5df.appspot.com/o/ha_long.png?alt=media&token=820c9948-929c-41a3-b376-7e1df5049ed2"
        )
        val city4 = City(
            4,
            "Vung Tau",
            "https://firebasestorage.googleapis.com/v0/b/travel-luxury-4f5df.appspot.com/o/vung_tau.png?alt=media&token=e20da326-27e3-4461-8748-4baff841f585"
        )
        val city5 = City(
            5,
            "Da Nang",
            "https://firebasestorage.googleapis.com/v0/b/travel-luxury-4f5df.appspot.com/o/da_nang.png?alt=media&token=db3cac9d-e959-4970-b406-b3d4c84477cc"
        )

        cityHotelList = listOf(city0, city1, city2, city3, city4, city5)
    }

}
